  ...
  return (
    <div id="auth-screen" className="flex items-center justify-center min-h-screen bg-[#0a0f1a] p-4">
      <div className="w-full max-w-lg bg-[#11192e] border border-slate-800 rounded-3xl p-8 md:p-10 shadow-2xl relative overflow-hidden">
        ...
        <div className="mt-6 p-4 bg-emerald-950/30 border border-emerald-800/40 rounded-2xl flex flex-col sm:flex-row items-center justify-between gap-3">
          <div className="flex items-center space-x-2.5">
            <Sparkles className="w-5 h-5 text-emerald-400 flex-shrink-0" />
            <div>
              <div className="text-xs font-bold text-emerald-300">Zkušební rychlý vstup</div>
              <div className="text-[11px] text-slate-400">Vyzkoušet aplikaci ihned bez registrace a hesel</div>
            </div>
          </div>
          <button
            id="btn-demo-quick-login"
            onClick={() => {
              setEmail('jan.kovar@firma.cz');
              setPassword('heslo123');
              login('jan.kovar@firma.cz', 'heslo123');
            }}
            className="w-full sm:w-auto px-4 py-2 bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold rounded-xl transition-all shadow-md shadow-emerald-900/30 flex items-center justify-center space-x-1.5"
          >
            <span>Hrát ihned jako host</span>
