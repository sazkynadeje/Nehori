...
export const AuthView: React.FC = () => {
  ...
  const [email, setEmail] = useState(() => {
    try {
      return localStorage.getItem('korpo_lingvo_last_email') || '';
    } catch {
      return '';
    }
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      try {
        localStorage.setItem('korpo_lingvo_last_email', email);
      } catch {}

      if (isRegister) {
        const res = await register(email, password, displayName, 'Tým');
        if (!res.success) {
      ...
      <div className="w-full max-w-lg bg-[#11192e] border border-slate-800 rounded-3xl p-8 md:p-10 shadow-2xl relative overflow-hidden">
        ...
        <div className="mb-6">
          <h2 className="text-2xl font-bold text-white tracking-tight">
            {isRegister ? 'Registrace nového hráče' : 'Přihlášení do týmové hry'}
          </h2>
          <p className="text-slate-400 text-sm mt-1">
            {isRegister
              ? 'Vytvoř si firemní profil a zapoj se do denní slovní výzvy se svým týmem.'
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          >
            <span>{isRegister ? 'Zaregistrovat se a vstoupit do hry' : 'Přihlásit se do hry'}</span>
